...
router.post("/tasks", async (req, res) => {
  try {
    ...
    const requiredFields = validateTaskData(taskData);
    if (!requiredFields.isValid) {
      return res.status(400).json({ success: false, error: requiredFields.error });
    }
    // 3. Get all existing tasks using getAllTasks()
    const tasks = await getAllTasks();
    // 4. Generate a new ID for the task
    const newId = tasks.length > 0 ? Math.max(...tasks.map((task) => parseInt(task.id))) + 1 : 1
...
router.delete("/tasks/:id", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message:`Task id: ${id} was deleted sucessfully`,
      deletedTask,
    });
    // Temporary response - remove this when you implement the above
    // res.status(501).json({
